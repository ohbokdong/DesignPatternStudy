...
## 가상 프록시(Virtual Proxy), 보호 프록시(Protection Proxy)

![week11/img6](https://github.com/ohbokdong/DesignPatternStudy/blob/master/summary/img/week11/img6.png?raw=true)

* **가상 프록시는 생성하는 데 많은 비용이 드는 객체를 대신하는 역할**
  * **객체가 필요하게 되기 전까지 객체의 생성을 미루게 해 주는 기능을 제공**
  * 객체 생선 전, 또는 객체 생성 도중에 객체를 대신하기도 함
  * 객체 생성이 완료되면 RealSubject에 요청을 직접 전달해 줌
  * CD 커버를 보여주는 예제(교재 코드리뷰)
...
* **보호 프록시**
  ...
  * InvocationHandler는 프록시에 대해서 호출되는 모든 메소드에 대해 응답하는 역할
    * Proxy에서 메소드 호출을 받으면 InvocationHandler에 진짜 작업을 부탁함
  * 객체마을 결혼 정보 예졔(교재 코드리뷰)
